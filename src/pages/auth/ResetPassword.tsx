...
export const ResetPassword: React.FC = () => {
  ...
  const handleSubmit = async (e: any) => {
    ...
    if (confirmPassword != password) {
      ...
      let data = await authService.ResetPasswordHandler({
        password: password,
        resetCode: code,
      });
        ...
        <div className="w-1/3 m-auto">
          ...
          <form className="space-y-4 flex flex-col" onSubmit={handleSubmit}>
            <p className="text-black font-bold">
              Veuillez inscrire un nouveau mot de passe pour votre compte
              EasyLife. Cela déconnectera toutes les sessions actives de votre
              compte.
            </p>
            <div className="relative">
              <label className="block text-black font-inter">
                Inscrivez un nouveau mot de passe{" "}
                <span className="text-customRouge">*</span>
              </label>
              <input
            ...
            <button
              type="submit"
              className="px-3 py-5 w-full bg-customBlue2 text-white py-3 rounded-2xl"
            >
          ...
          <p className="text-center mt-4">
            Besoin d'aide?{" "}
            <a href="/" className="text-customBlue2 font-semibold">
              Contactez-nous
            </a>
          </p>
        </div>
