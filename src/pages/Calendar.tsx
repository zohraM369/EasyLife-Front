...
export const Calendar: React.FC<CalendarProps> = ({ tasks }) => {
  ...
  const calendarDays = Array.from({ length: 42 }, (_, i) => {
    ...
    const eventsForDay = tasks.filter(
      (e) => convertDate(e.date).toISOString().split("T")[0] === dateString
    );
    ...
    return (
      <div key={i} className="h-15 border p-1 relative">
        ...
        {eventsForDay.length > 0 ? (
          ...
          >
            <div
              className={`w-full h-4 ${getTaskColor(
                eventsForDay[0].type
              )} flex py-5 items-center justify-center rounded text-white text-xs`}
            >
              <h4 className="text-center font-medium">
                {eventsForDay.length > 1
                  ? `${eventsForDay.length} taches`
                  : eventsForDay[0].title}
                <br />
                <span>{eventsForDay[0].time}</span>
              </h4>
            </div>
            <div className="flex items-center justify-center">
              <FaCircle
                size={"1.5vh"}
                color={getStatusColor(eventsForDay[0].status)}
              />
              <img
                className="h-3 rounded"
                src={`http://localhost:8000${eventsForDay[0].user_id.image}`}
              />
          ...
          <div
            className={`w-full h-4 flex items-center justify-center p-5 rounded text-white text-xs`}
          >
