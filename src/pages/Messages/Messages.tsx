  ...
  return (
    <div className="min-h-screen flex flex-col items-center bg-gray-100 py-8">
        ...
        <section className="mb-8">
          <div className='flex items-center justify-around'><h2 className="text-3xl font-bold text-gray-800 mb-6">Liste des discussions</h2>
           <div className="mb-4">
         ...
         {uniquePeopleTalkedWith.length>0 && uniquePeopleTalkedWith.map(el=>{
          return <button onClick={()=>handleGetMessages(user._id, (el.recipientId._id == user._id) ? el.user_id._id :el.recipientId._id )} className='text-white bg-blue-500 mx-2 px-2 py-4 rounded'>
           <img className='h-10 rounded' src={
             user._id == el.user_id._id ? `http://localhost:8000${el.recipientId.image}` :   `http://localhost:8000${el.user_id.image}` 
             }  />
           <span>{
             user._id == el.user_id._id ? el.recipientId.name : el.user_id.name
             }</span> 
            </button>
         })}
          </section>
        <section>
            ...
            {messages.length>0 && messages.map((message) => (
              <li key={message._id} className="shadow-lg flex items-center p-6 bg-gray-50 rounded-lg shadow-sm">
                <div className="flex-shrink-0 mr-4">
                  <img src={`http://localhost:8000${message.user_id?.image}`} alt={user?.name} className="w-16 h-16 rounded-full object-cover" />
                </div>
                <div className="flex-1">
                  ...
                  <h4 className="text-xl font-semibold text-gray-700">{message.user_id.name == user.name ? "Moi":message.user_id.name}</h4>
           ...
           {messages.length>0 && <div className='flex justify-center'> <span className='mx-1'>Répondre</span> 
           <input onChange={(e)=>setTextMessage(e.target.value)} className='border-1 border' type="text"  /> 
           <button onClick={() => handleRespond((messages[0].recipientId._id == user._id) ? messages[0].user_id._id :messages[0].recipientId._id )} className="ml-4 text-blue-500 hover:text-blue-700">
                  <FaMessage />
                </button> </div>} 
          </ul>
        </section>
