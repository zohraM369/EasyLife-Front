...
const todayUTCString = new Date(Date.UTC(today.getUTCFullYear(), today.getUTCMonth(), today.getUTCDate()))
  .toISOString()
  .split('T')[0];
    ...
    <div className="mt-3 col-span-12 rounded-sm border border-stroke bg-white py-6 shadow-default dark:border-strokedark dark:bg-boxdark xl:col-span-4">
   ...
   
      {
        events.length > 0  ?events.map(element=> {
          return    <div className="border-2 border-blue-200 mx-1 mb-1  text-center border-black-100 shadow text-black">
            <Card title={element.title} time={element.time} color={getTaskColor(element.type)}  />
           </div>
        }):  <div className="text-center border shadow text-black"><p>Aucune tàche Ajourd'hui</p></div>
      }
