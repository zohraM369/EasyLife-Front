      ...
      <div className="container mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {Object.keys(groupedTasks).map((type) => (
            ...
            >
              <h2
                className={`text-xl font-bold ${getTaskColor(
                  type
                )} flex items-center`}
              >
              ...
              {groupedTasks[type].map((task) => (
                <div
                  key={task._id}
                  className="mt-4 p-4 bg-gray-50 border border-gray-300 rounded-lg shadow-sm cursor-pointer hover:bg-gray-100 transition duration-300"
                >
                  <div className="flex justify-between items-center">
                    ...
                    <h4
                      onClick={() =>
                        navigate(`/dashboard/task_management/${task._id}`)
                      }
                    >
                      Voir les détails
                    </h4>
                  ...
                  <span
                    className={`block mt-2 p-1 border rounded-md ${getStatusColor(
                      task.status
                    )} text-sm font-medium`}
                  >
                    {task.status}
