...
export const authService = {
  ...
  updateEmail: async (userId: string, newEmail: string, password: string) => {
    const response = await axios.post(`${API_URL}/auth/update_email`, {
      userId: userId,
      password: password,
      newEmail: newEmail,
    });
  ...
  updateCity: async (userId: string, newCity: string, password: string) => {
    const response = await axios.post(`${API_URL}/auth/update_city`, {
      userId: userId,
      newCity: newCity,
      password: password,
    });
  ...
  updatePassword: async (
    ...
    const response = await axios.post(`${API_URL}/auth/update_password`, {
      userId: userId,
  ...
  updateName: async (userId: string, newName: string, password: string) => {
    const response = await axios.post(`${API_URL}/auth/update_name`, {
      userId: userId,
      newName: newName,
      password: password,
    });
  ...
  updateImage: async (user_id: string, profileImage: Blob) => {
    ...
    try {
      const response = await axios.post(
        `${API_URL}/auth/update_user_image`,
        formData,
        {
          headers: {
  ...
  delete_user: async (user_id: string) => {
    const response = await axios.delete(
      `${API_URL}/auth/delete_user/${user_id}`
    );
    return response.data;
  },
};
